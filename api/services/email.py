"""
Email Service for Florida Prod Market LLC
...
class EmailService:
    ...
    def __init__(self):
        ...
        self.backend = get_clean_env("EMAIL_BACKEND", "")
        if not self.backend or self.backend == "console":
            if get_clean_env("RESEND_API_KEY"):
                self.backend = "resend"
            else:
                self.backend = "console"
                
        self.from_email = get_clean_env("FROM_EMAIL", "floridaprod@proton.me")
        self.from_name = get_clean_env("FROM_NAME", "Florida Prod Market LLC")
    ...
    def send_order_confirmation(self, order_data: Dict[str, Any]) -> bool:
        ...
        <body style="font-family: 'Inter', 'Segoe UI', Arial, sans-serif; line-height: 1.6; color: #1e293b; max-width: 600px; margin: 0 auto; padding: 0; background-color: #f8fafc;">
            <div style="background: linear-gradient(135deg, #0f172a 0%, #3b82f6 100%); padding: 40px 20px; text-align: center;">
                <h1 style="color: #ffffff; margin: 0; font-size: 32px; font-weight: 800; letter-spacing: -0.025em;">Order Confirmed!</h1>
                <p style="color: #bfdbfe; margin: 12px 0 0 0; font-size: 18px; font-weight: 500;">Thank you for choosing Florida Prod Market LLC</p>
            </div>
            ...
            <div style="padding: 30px; background-color: #ffffff; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                <div style="background: #eff6ff; padding: 24px; border-radius: 12px; margin-bottom: 32px; border: 1px solid #dbeafe; text-align: center;">
                    ...
                    <p style="margin: 0; font-size: 36px; font-weight: 800; color: #3b82f6; letter-spacing: 1px;">{order.get('order_number', 'N/A')}</p>
                ...
                <div style="background: #f8fafc; padding: 20px; border-radius: 12px; border-left: 4px solid #3b82f6; margin-bottom: 32px;">
                    <p style="margin: 0; color: #1e293b; font-weight: 700; font-size: 16px;">Status: Pending Processing</p>
                    <p style="margin: 8px 0 0 0; color: #64748b; font-size: 14px; line-height: 1.5;">Our team has received your order and will begin processing it shortly. You will receive further updates as the shipment progresses.</p>
                </div>
                
                <div style="text-align: center;">
                    <a href="#" style="display: inline-block; background-color: #0f172a; color: #ffffff; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 700; transition: background-color 0.2s;">Track My Order</a>
                </div>
                
                <div style="text-align: center; margin-top: 48px; padding-top: 24px; border-top: 1px solid #f1f5f9;">
                    <p style="color: #94a3b8; font-size: 13px; margin: 0; line-height: 1.6;">
                        <strong>Florida Prod Market LLC</strong><br>
                        Premium Trucking & Logistics Services<br>
