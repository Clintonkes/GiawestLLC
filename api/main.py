"""
Florida Prod Market LLC API - Main Application Entry Point
FastAPI backend for the Florida Prod Market LLC trucking logistics application.
...
# Route imports
from api.routes import orders, trucks, contact, reviews, admin, uploads
from api.seed_fleet import seed_fleet
...
async def lifespan(app: FastAPI):
    ...
    try:
        logger.info("Seeding initial fleet...")
        seed_fleet()
    except Exception as e:
        logger.error(f"Failed to seed fleet: {e}")
    
    # Restore missing default images and update DB
    try:
        ...
        sources = {
            "semi.png": r"/mnt/c/Users/user/.gemini/antigravity/brain/3eeca1dd-b57e-4817-a265-be033160874b/clean_semi_truck_v2_1774624287574.png",
            "van.png": r"/mnt/c/Users/user/.gemini/antigravity/brain/3eeca1dd-b57e-4817-a265-be033160874b/clean_delivery_van_v2_1774624633373.png",
            "reefer.png": r"/mnt/c/Users/user/.gemini/antigravity/brain/3eeca1dd-b57e-4817-a265-be033160874b/reefer_truck_placeholder_1774623031204.png"
        }
        
        for name, src in sources.items():
            dest = UPLOAD_DIR / name
            if not dest.exists() and os.path.exists(src):
                shutil.copy2(src, dest)
                logger.info(f"Restored missing fleet image: {name}")
        db.close()
    except Exception as e:
        logger.error(f"Failed to auto-restore fleet assets: {e}")
...
app = FastAPI(
    title="Florida Prod Market LLC API",
    description="Backend API for Florida Prod Market LLC - Trucking & Logistics Services",
    version="1.0.0",
    docs_url="/api/docs",
    redoc_url="/api/redoc",
    lifespan=lifespan
...
async def api_root():
    ...
    return {
        "message": "Florida Prod Market LLC API",
        "version": "1.0.0",
        "docs": "/api/docs",
        "health": "/api/health"
    }
...
# Mount /api/uploads for user-uploaded content (Available in both dev and prod)
...
if FRONTEND_PATH.exists():
    ...
    async def root():
        ...
        return {
            "message": "Welcome to Florida Prod Market LLC API (Frontend not built)",
            "version": "1.0.0",
            "docs": "/api/docs",
            "health": "/api/health"
        }
...
if __name__ == "__main__":
    ...
    new_url = f"NEXT_PUBLIC_API_URL=http://localhost:{port}/api"
    try:
        lines = []
    ...
    print("\n" + "="*60)
    print("🚀 Florida Prod Market LLC API Server")
    print("="*60)
