...
async def update_order_status(
    ...
    if assigned_truck_id is not None:
        if previous_truck_id and previous_truck_id != assigned_truck_id:
            old_truck = db.query(Truck).filter(Truck.id == previous_truck_id).first()
            if old_truck:
                old_truck.status = TruckStatus.AVAILABLE
        order.assigned_truck_id = assigned_truck_id
        
    final_truck_id = order.assigned_truck_id
    
    # Handle truck status based on order status changes
    if new_status in ["completed", "cancelled"]:
        if final_truck_id:
            truck = db.query(Truck).filter(Truck.id == final_truck_id).first()
            if truck:
                truck.status = TruckStatus.AVAILABLE
    elif new_status in ["assigned", "in_progress"]:
        if final_truck_id:
            truck = db.query(Truck).filter(Truck.id == final_truck_id).first()
            if truck:
